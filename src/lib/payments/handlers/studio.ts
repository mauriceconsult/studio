...
export async function handleStudio(payload: MoMoWebhookPayload): Promise<void> {
  
  // MoMo webhook payload shape (adjust to your provider's actual format)
  const { referenceId, status } = payload as {
    referenceId: string;
  ...
  if (topUp) {
    ...
    if (topUp.jobId) {
      ...
      if (course) {
        ...
        console.warn(
          "[momo-webhook] jobId not found in Course or Video:",
          topUp.jobId,
        );
      }
    }
