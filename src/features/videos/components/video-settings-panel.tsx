    ...
    <div className="space-y-4 p-4">
      <div className="space-y-1">
        ...
        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium capitalize
          ${video.status === "done" ? "bg-emerald-50 text-emerald-800"
      ...
      {(video.status === "processing" || video.status === "pending") && (
        <div className="space-y-1">
          ...
          <div className="h-1.5 overflow-hidden rounded-full bg-border">
            <div className="h-full bg-amber-400 transition-all duration-500" style={{ width: `${video.progress}%` }} />
          </div>
          <p className="text-xs tabular-nums text-muted-foreground">{video.progress}%</p>
        </div>
      )}
      {video.durationSeconds && (
        <div className="space-y-1">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-widest">Duration</p>
          <p className="text-sm text-foreground">{video.durationSeconds}s</p>
        </div>
      )}
      {video.errorMessage && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-3">
          <p className="text-xs text-red-700">{video.errorMessage}</p>
        </div>
      )}
      <div className="space-y-1">
...
function VideoHistory() {
  ...
  const { data: videos } = useSuspenseQuery(trpc.videos.getAll.queryOptions({}));
      ...
      {videos.map((v) => (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground capitalize">
            {v.status} · {formatDistanceToNow(new Date(v.createdAt), { addSuffix: true })}
          </p>
        </button>
      ))}
    </div>
...
export function VideoSettingsPanel({ video }: { video: Video }) {
  return (
